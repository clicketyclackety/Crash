...
namespace Crash.Views
{
    ...
    public class CrashPanel : Panel, IPanel
    {
        readonly uint m_document_sn = 0;

        /// <summary>
        /// Provide easy access to the CrashPanel.GUID
        /// </summary>
        public static System.Guid PanelId => typeof(CrashPanel).GUID;

        /// <summary>
        /// Required public constructor with NO parameters
        /// </summary>
        public CrashPanel(uint documentSerialNumber)
        {
            m_document_sn = documentSerialNumber;

            Title = GetType().Name;

            var start_button = new Button { Text = "Start Shared Model..." };
            start_button.Click += (sender, e) => OnStartButton();


            /// var document_sn_label = new Label() { Text = $"Document serial number: {documentSerialNumber}" };

            var layout = new DynamicLayout { DefaultSpacing = new Size(5, 5), Padding = new Padding(10) };
            layout.AddSeparateRow(start_button, null);
        ...
        protected void OnStartButton()
        {
            ...
            Dialogs.ShowMessage("Starting Shared Model!", Title);
        }
